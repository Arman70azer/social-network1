...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (!wsConnect) {
      ...
      setTimeout(() => {
        sendRequestToWebsocket(wsConnect, {
          Origin: "home",
          Nature: "enterToHome",
          User: "Arman",
        });
    ...
    if (event.key === "Enter") {
      ...
      if (enterComment != "" && seeThisPostCommentaries != "") {
        ...
        sendRequestToWebsocket(wsConnect, {
          ...
          Nature: "newComment",
          User: "Arman",
          ObjetcOfRequest: seeThisPostCommentaries,
        });
      }
    }
  };
    ...
    if (data && wsConnect) {
      ...
      wsConnect.onmessage = (event) => {
        ...
        if (receivedMessage.Accept) {
          ...
          let postTarget;
          if (data.Posts) {
            postTarget = data.Posts.find(
              (post) => post.Titre === receivedMessage.Post
            );
          }
          if (postTarget) {
            ...
            if (receivedMessage.Nature === "New-like") {
              ...
              if (receivedMessage.ObjectOfRequest === "add") {
                ...
                if (receivedMessage.OtherLikeDislike) {
                  postTarget.Dislikes = postTarget.Dislikes.filter(
                    (dislikePost) => dislikePost.User !== receivedMessage.User
                  );
                }
              } else if (receivedMessage.ObjectOfRequest === "remove") {
                postTarget.Likes = postTarget.Likes.filter(
                  (like) => like.User !== receivedMessage.User
                );
              }
              ...
              if (receivedMessage.ObjectOfRequest === "add") {
                ...
                if (receivedMessage.OtherLikeDislike) {
                  postTarget.Likes = postTarget.Likes.filter(
                    (likePost) => likePost.User !== receivedMessage.User
                  );
                }
              } else if (receivedMessage.ObjectOfRequest === "remove") {
                postTarget.Dislikes = postTarget.Dislikes.filter(
  ...
  return (
    <div>
      {data.Events && wsConnect != null ? (
        <DashboardTop events={data.Events} ws={wsConnect} />
      ) : (
        <DashboardTop />
      )}
          ...
          data.Posts.map((post, index) => (
            ...
            >
              <div className={styles.alineProfilPost}>
                ...
                <div className={styles.typePost}>{post.Type} Post</div>
              </div>
              <div className={styles.postContent}>
                <div className={styles.titlePost}>{post.Content}</div>
                {post.ImageName && (
              ...
              <div className={styles.postFooter}>
                  ...
                  <button
                    ...
                    style={{
                      color:
                        post.Likes &&
                        post.Likes.some(
                          (likeOrDislike) => likeOrDislike.User === "Arman"
                        )
                          ? "blue"
                          : "white",
                    }}
                ...
                <div className={styles.commentaires}>
                  <button onClick={() => seeCommentaries(post.Titre)}>
                    {post.Commentaries && post.Commentaries.length > 0
                      ? `See commentaries (${post.Commentaries.length})`
                      : "See commentaries"}
                  </button>
                </div>
                <div className={styles.datePost}>{post.Date}</div>
              </div>
                ...
                <div className={styles.areaCommentary}>
                  <div className={styles.inputComment}>
                    <input
                      ...
                      onKeyDown={(event) => submitCommentary(event)}
                      value={enterComment}
                  ...
                  {post.Commentaries &&
                    post.Commentaries.map((comment, index) => (
                      <div className={styles.commentsContent} key={index}>
                        <Link href={`/profil/` + comment.Author.Nickname}>
      ...
      <div className={styles.dashboardBottomPage}>
        <button className={styles.buttonPostPublic} onClick={onlyPublicPosts}>
          Publics Posts
        </button>
        <button
          className={styles.buttonPostPrivates}
          onClick={onlyPrivatePosts}
        >
          Privates Posts
        </button>
        <button className={styles.buttonPostsAll} onClick={resetDataToOrigine}>
          {" "}
          All Posts{" "}
        </button>
        <Link href="/createPost" className={styles.buttonCreatePost}>
          Add New Post or Event [+]
        </Link>
      </div>
    </div>
  );
